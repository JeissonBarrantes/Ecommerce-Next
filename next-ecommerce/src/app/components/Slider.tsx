...
const Slider = () => {
  ...
  return <div className="h-[calc(100vh-80px)]">
    <div className="w-max h-full flex transition-all ease-in-out duration-500 "
      ...
      {slides.map(slide => (
        ...
        key={slide.id}>
            ...
            <div className="h-1/2 xl:w-1/2 xl:h-full flex flex-col items-center justify-center gap-8 2xl:gap-12 text-center"> 
              ...
              <Link href={slide.url} className="bg-black text-white py-4 px-8 rounded-md">Shop Now</Link>
            </div>
            {/* IMAGE CONTAINER */}
    ...
    <div className="absolute m-auto left-1/2 bottom-8 flex gap-4">

      {
        slides.map((slide, index) => (
          <div className={`w-3 h-3 rounded-full ring-1 ring-gray-600 cursor-pointer flex items-center justify-center ${current === index ? "scale-50": ""}`} 
          key={slide.id}
